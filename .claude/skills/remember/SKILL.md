...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope telek
...
scope:** the default personal scope is **private** (`--scope telek
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `telek`). Private records are served only to a recall in the same
