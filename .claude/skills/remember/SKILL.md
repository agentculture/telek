...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope telegram-agent
...
scope:** the default personal scope is **private** (`--scope telegram-agent
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `telegram-agent`). Private records are served only to a recall in the same
